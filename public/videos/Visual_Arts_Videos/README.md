# ╔══════════════════════════════════════════════════════════════╗
# ║   Visual Arts — Video File  (1 video slot)                  ║
...
## EXACT FILENAME TO USE  (one file only)
  va_video_01.mp4        ←  recommended
  va_video_01.webm       ←  alternative
  va_video_01.mov        ←  alternative
  (formats tried in that order — first one found is used)

  Title shown on site:  "Visual Arts Exhibition Showcase"
...
## NETLIFY DEPLOYMENT OPTIONS
  ...
  OPTION A — Small file (< 50 MB):
    git add public/videos/Visual_Arts_Videos/va_video_01.mp4
    git commit -m "Add Visual Arts video"
    git push
  ...
  OPTION C — YouTube only:
    Update youtubeId in departments.ts → visualArtsVideos. Done.
